...
COPY scripts/patch-openclaw-canvas-cookie-auth.py /tmp/patch-openclaw-canvas-cookie-auth.py
RUN set -eu; \
    OPENCLAW_DIR="$(npm root -g)/openclaw"; \
    if [ -d "$OPENCLAW_DIR/dist" ]; then \
      python3 /tmp/patch-openclaw-canvas-cookie-auth.py "$OPENCLAW_DIR/dist"; \
    fi; \
    rm -f /tmp/patch-openclaw-canvas-cookie-auth.py
